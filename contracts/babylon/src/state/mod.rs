...
pub fn handle_btc_timestamp(
    ...
    debug!(deps, "CONTRACT: handle_btc_timestamp: starting to process BTC timestamp");
    ...
    if let Some(btc_headers) = btc_ts.btc_headers.as_ref() {
        ...
        if !btc_headers.headers.is_empty() {
            debug!(deps, "CONTRACT: handle_btc_timestamp: creating BTC headers message");
            wasm_msg = Some(
                ...
                .map_err(|e| {
                    ...
                    error!(deps, "CONTRACT: handle_btc_timestamp: {}", err_msg);
                    StdError::generic_err(err_msg)
                })?,
            );
        }
    } else {
        debug!(deps, "CONTRACT: handle_btc_timestamp: no BTC headers found");
    }
    ...
    if babylon_epoch_chain::is_initialized(deps) {
        debug!(deps, "CONTRACT: handle_btc_timestamp: Babylon epoch chain is initialized, handling epoch and checkpoint");
